...
def amino2domain(infile, outfile, hmm_dir, cutoff):
    ...
    for record in aln:
        id = str(record.id)
        seq = str(record.seq)
        ####### THESE TWO LINES FOR HRH ONLY ######## hard-coded in, only. sry.
        protid = "_".join(id.split("_")[:2])
        hmmFile = hmm_dir + protid + '.txt'
        #############################################
        #print id
...
infile = sys.argv[1]
outfile = sys.argv[2]
hmm_dir = "/Users/sjspielman/Dropbox/Amine/HRH/gpcrhmm_structures/"
cutoff = 0.8
